...
export function AccommodationSelection({ accommodationType, onComplete, onBack, initialData, profile }: AccommodationSelectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Promise.resolve(onComplete(selection));
    } catch (err: any) {
  ...
  const handleCodeVerified = async () => {
    ...
    try {
      setIsSubmitting(true);
      await Promise.resolve(onComplete({ ...selection, isPaired: true }));
    } catch (err: any) {
      setFormError(toUserMessage(err, { feature: "generic", action: "create" }));
    } finally {
      setIsSubmitting(false);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            >
              {isSubmitting ? 'Saving…' : (accommodationType === 'hostel' && selection.price
