...
export function EventSelection({
  ...
  const filteredEvents = isYATEligible
    ? mockEvents.filter((event) => event.id === "yat-campmeeting26")
    : mockEvents.filter((event) => event.id !== "yat-campmeeting26");
  ...
  const handleSelect = (event: Event) => {
    ...
    if (typeof onSelectEvent === "function") {
      onSelectEvent(event.id, event.name);
      return;
    }
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      console.error(
        "EventSelection: Missing handler. Provide onSelectEvent(eventId,eventName) or onContinue()."
      );
    }
  };

  return (
    <div className="flex-1 overflow-auto pt-8 lg:pt-[150px] px-4 lg:pr-8 lg:pb-8 lg:pl-8">
